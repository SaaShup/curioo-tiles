...
const { DEFAULT_THEME, TILE_ZOOM_RANGE } = require("./config");
const { tileRequests, tileRenderDuration } = require("./metrics");
const { generateTile } = require("./renderer");

function isZoomSupported(z) {
  const [minZoom, maxZoom] = TILE_ZOOM_RANGE;
  return z >= minZoom && z <= maxZoom;
}

function getUnsupportedZoomMessage() {
  const [minZoom, maxZoom] = TILE_ZOOM_RANGE;
  if (minZoom === maxZoom) {
    return `Only zoom ${minZoom} supported`;
  }
  return `Only zooms ${minZoom}-${maxZoom} supported`;
}
...
function createTileHandler(previewThemes) {
  return async function handleTileRequest(req, res, themeName) {
    ...
    if (!isZoomSupported(z)) {
      return res.status(404).send(getUnsupportedZoomMessage());
    }
...
module.exports = { createTileHandler, isZoomSupported, getUnsupportedZoomMessage };
