...
function getZonesForBbox(bbox) {
  const epsilon = 1e-12;
  const minLatIndex = Math.floor(bbox.latMin / ZONE_SIZE_DEGREES);
  const maxLatIndex = Math.floor((bbox.latMax - epsilon) / ZONE_SIZE_DEGREES);
  const minLonIndex = Math.floor(bbox.lonMin / ZONE_SIZE_DEGREES);
  const maxLonIndex = Math.floor((bbox.lonMax - epsilon) / ZONE_SIZE_DEGREES);
  const zones = [];

  for (let latIndex = minLatIndex; latIndex <= maxLatIndex; latIndex++) {
    for (let lonIndex = minLonIndex; lonIndex <= maxLonIndex; lonIndex++) {
      zones.push(getZoneForLatLon(
        latIndex * ZONE_SIZE_DEGREES,
        lonIndex * ZONE_SIZE_DEGREES
      ));
    }
  }
...
function getZonesFromTile(z, x, y) {
  return getZonesForBbox(tile2bbox(x, y, z));
}
